...
{  
   ...
   int1 ret;                                     //ACK sorgulama için değişken
   
   output_bit(SS1_A0, 1);                        //DAC A0 pini adres ayarı
   output_bit(SS2_A0, 0);
   
   output_high(SDA);                             //Başlangıçta SDA ve SCL çıkışlarını HIGH duruma getir
   output_high(SCL);
   
   i2c_start();                                      //Başlangıç bitini yolla
